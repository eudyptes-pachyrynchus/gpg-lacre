...
if (
    "database" in cfg
    and "enabled" in cfg["database"]
    and cfg["database"]["enabled"] == "yes"
    and "name" in cfg["database"]
    and "host" in cfg["database"]
    and "username" in cfg["database"]
    and "password" in cfg["database"]
):
    ...
    for row in result_set:
        ...
        cursor.execute(
            "DELETE FROM gpgmw_keys WHERE email = %s AND id != %s",
            (
                row[2],
                row[1],
            ),
        ...
        if row[
            0
        ].strip():  # we have this so that user can submit blank key to remove any encryption
            if GnuPG.confirm_key(row[0], row[2]):
                GnuPG.add_key(cfg["gpg"]["keyhome"], row[0])  # import the key to gpg
                cursor.execute(
                    "UPDATE gpgmw_keys SET status = 1 WHERE id = %s", (row[1],)
                )  # mark key as accepted
                appendLog("Imported key from <" + row[2] + ">")
                if "send_email" in cfg["cron"] and cfg["cron"]["send_email"] == "yes":
                    send_msg(
                        "PGP key registration successful",
                        "registrationSuccess.md",
                        row[2],
                    )
            else:
                cursor.execute(
                    "DELETE FROM gpgmw_keys WHERE id = %s", (row[1],)
                )  # delete key
                appendLog("Import confirmation failed for <" + row[2] + ">")
                if "send_email" in cfg["cron"] and cfg["cron"]["send_email"] == "yes":
                    send_msg(
                        "PGP key registration failed", "registrationError.md", row[2]
                    )
        else:
