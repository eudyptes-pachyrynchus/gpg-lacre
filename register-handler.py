...
if __name__ == "__main__":
    ...
    for msg_part in register_msg.walk():
        if (
            msg_part.get_content_type().lower() == "application/pkcs7-signature"
            or msg_part.get_content_type().lower() == "application/x-pkcs7-signature"
        ):
            sign_type = "smime"
            sign_part = msg_part
            break
        ...
        # 	break
            ...
            "-----BEGIN PGP PUBLIC KEY BLOCK-----" in msg_part.get_payload()
            and "-----END PGP PUBLIC KEY BLOCK-----" in msg_part.get_payload()
        ):
    ...
    if sign_part == None:
        ...
        failure_msg = file(
            cfg["mailregister"]["mail_templates"] + "/registrationError.md"
        ).read()
...
-----END PKCS7-----
        ...
        processed_from_addr = (
            splitted_from_addr[0] + "@" + splitted_from_addr[1].lower()
        )
        ...
        success_msg = file(
            cfg["mailregister"]["mail_templates"] + "/registrationSuccess.md"
        ).read()
        ...
        if r.status_code != 200:
            ...
            error_msg = file(
                cfg["mailregister"]["mail_templates"] + "/gpgmwFailed.md"
            ).read()
