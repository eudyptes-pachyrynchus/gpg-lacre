...
verbose = (
    "logging" in cfg
    and "verbose" in cfg["logging"]
    and cfg["logging"].get("verbose") == "yes"
)
...
def gpg_decrypt(raw_message, recipients):
    ...
    for to in recipients:
        if to in list(keys.values()) and not get_bool_from_cfg(
            "default", "dec_keymap_only", "yes"
        ):
            gpg_to.append(to)
        # Is this recipient defined in regex for default decryption?
            ...
            if not cfg["dec_keymap"][to] in keys:
                log(
                    "Key '%s' in decryption keymap not found in keyring for email address '%s'. Won't decrypt."
                    % (cfg["dec_keymap"][to], to)
                )
                # Avoid unwanted encryption if set
                if to in list(keys.values()) and get_bool_from_cfg(
                    "default", "failsave_dec", "yes"
                ):
                    noenc_to.append(to)
                else:
                    ungpg_to.append(to)
...
def gpg_decrypt_all_payloads(message):
    ...
    if (
        not (message.get_param("protocol") is None)
        and message.get_param("protocol") == "application/pgp-encrypted"
...
def decrypt_mime(decrypted_message):
    ...
    if (
        "-----BEGIN PGP MESSAGE-----" in msg_content
        and "-----END PGP MESSAGE-----" in msg_content
    ):
        start = msg_content.find("-----BEGIN PGP MESSAGE-----")
        end = msg_content.find("-----END PGP MESSAGE-----")
        decrypted_payload, decrypt_success = decrypt_payload(
            msg_content[start : end + 25]
        )
        ...
        if decrypt_success:
            ...
            if not (decrypted_payload.get("Content-Disposition") is None):
                if not (decrypted_message.get("Content-Disposition") is None):
                    decrypted_message.replace_header(
                        "Content-Disposition",
                        decrypted_payload.get("Content-Disposition"),
                    )
                else:
                    decrypted_message.set_param(
                        decrypted_payload.get("Content-Disposition"),
                        "",
                        "Content-Disposition",
                    )
            ...
            if decrypted_payload.is_multipart():
                ...
                decrypted_message.set_payload(list())
                decrypted_message = generate_message_from_payloads(
                    decrypted_payload, decrypted_message
                )
                decrypted_message.preamble = (
...
def decrypt_inline_with_attachments(payloads, success, message=None):
    ...
    for payload in payloads.get_payload():
        if type(payload.get_payload()) == list:
            ...
            if (
                pgp_inline_tags
                or not (attachment_filename is None)
                and not (re.search(".\.pgp$", attachment_filename) is None)
            ):
                if pgp_inline_tags:
                    start = msg_content.find("-----BEGIN PGP MESSAGE-----")
                    end = msg_content.find("-----END PGP MESSAGE-----")
...
-----END PGP MESSAGE-----"""
                ...
                if decrypt_success:
                    ...
                    if not (attachment_filename is None):
                        ...
                        payload.set_param(
                            "filename", attachment_filename, "Content-Disposition"
                        )
    ...
    for to in recipients:
        ...
        if get_bool_from_cfg("enc_keymap", to):
            ...
            if cfg["enc_keymap"][to] in keys:
                ...
                log(
                    "Key '%s' in encrypt keymap not found in keyring for email address '%s'."
        ...
        if len(splitted_to) > 1:
            ...
            if get_bool_from_cfg("enc_domain_keymap", domain):
                log(
                    "Encrypt domain keymap has key '%s'" % cfg["enc_dec_keymap"][domain]
                )
                # Check we've got a matching key!
                if cfg["enc_domain_keymap"][domain] in keys:
                    ...
                    log(
                        "Key '%s' in encrypt domain keymap not found in keyring for email address '%s'."
        ...
        for rcpt in gpg_to:
            ...
            if get_bool_from_cfg("pgp_style", rcpt[0], "mime"):
                ...
                if get_bool_from_cfg("pgp_style", rcpt[0]):
                    log(
                        "Style %s for recipient %s is not known. Use default as fallback."
...
def encrypt_all_payloads_inline(message, gpg_to_cmdline):
    ...
    for payload in message.get_payload():
        if type(payload.get_payload()) == list:
            encrypted_payloads.extend(
                encrypt_all_payloads_inline(payload, gpg_to_cmdline)
            )
        else:
...
def encrypt_all_payloads_mime(message, gpg_to_cmdline):
    ...
    if type(message.get_payload()) == str:
        ...
        additionalSubHeader = ""
        if "Content-Type" in message and not message["Content-Type"].startswith(
            "multipart"
        ):
...
def encrypt_payload(payload, gpg_to_cmdline, check_nested=True):
    ...
    if (
        check_nested
        and "-----BEGIN PGP MESSAGE-----" in raw_payload
        and "-----END PGP MESSAGE-----" in raw_payload
    ):
        if verbose:
    ...
    gpg = GnuPG.GPGEncryptor(
        cfg["gpg"]["keyhome"], gpg_to_cmdline, payload.get_content_charset()
    )
    ...
    isAttachment = (
        payload.get_param("attachment", None, "Content-Disposition") is not None
    )
    ...
    if isAttachment:
        ...
        if filename:
            ...
            if not (payload.get("Content-Type") is None) and not (
                payload.get_param("name") is None
            ):
...
def smime_encrypt(raw_message, recipients):
    ...
    if not get_bool_from_cfg("smime", "cert_path"):
        log(
            "No valid path for S/MIME certs found in config file. S/MIME encryption aborted."
        )
        return recipients
...
def send_msg(message, recipients):
    ...
    if recipients:
        if not (
            get_bool_from_cfg("relay", "host") and get_bool_from_cfg("relay", "port")
        ):
        ...
        if (
            "relay" in cfg
            and "starttls" in cfg["relay"]
            and cfg["relay"]["starttls"] == "yes"
        ):
            smtp.starttls()
